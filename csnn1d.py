...
class CSNN1d(Model):
    ...
    def build_model(self):
        ...
        #self.final_block = [
                            #layer.Linear(in_channels= self.config.channels[i-1] , out_channels = self.config.channels[i], kernel_size = self.config.kernel_sizes[i],
                            #             stride = self.config.strides[i], bias = self.config.bias, step_mode='m')
        #                    ]

        #self.blocks.append(nn.Sequential(*self.final_block))
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(2)                      # add channels dimension  (batch, time, channels, neurons)
